...
public:
    ...
    friend Matrix applyOperands(Matrix input, std::vector<QubitIndex> const& operands);
...
inline Matrix applyOperands(Matrix input, std::vector<QubitIndex> const& operands) {
    assert(std::ranges::all_of(operands, [&](auto const& index) {
        return index.value < operands.size();
    }));
    ...
    for (std::uint64_t i = 0; i < (1UL << operands.size()); ++i) { // FIXME: refactor this.
        ...
        assert((i == ((1UL << operands.size()) - 1)) || (inputRowOrColumn & (1UL << operands[operands.size() - rightOnes - 1].value)) == 0);

        if (i < (1UL << operands.size()) - 1) { // FIXME: ugly
            inputRowOrColumn &= masks[rightOnes];
            assert(rightOnes < operands.size());
            inputRowOrColumn |= 1UL << operands[operands.size() - rightOnes - 1].value;
        }
    }
